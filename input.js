...
window.addEventListener('pageshow', function () {
    ...
    if (seconds > 30 * 60) { // 超过30分钟，过期
        read();
        return;
    }

    frPost(frs.read_commit, {uid, ext}, function (res) {
...
function read() {
    ...
    frPost(frs.read_get, {uid, ext, img}, function (res) {
        var result = res.result;
        var status = result.status;
        if (status === 10) {
            setStorage('task', JSON.stringify({ts: +new Date}));
